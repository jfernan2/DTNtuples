...
m_outFile(outFileName,"RECREATE"), DTNtupleBaseAnalyzer(inFileName), quality_(quality), DM_(DM)
{
  ...
  m_maxSegTrigDEta = 0.15;
  //MBRadius= {402.0,488.5,597.5}; 
  m_maxSegT0 = 15;
...
void DTNtupleTPGSimAnalyzer::fill()
{
  ...
  for (std::size_t iGenPart = 0; iGenPart < gen_nGenParts; ++iGenPart)
  {
    ...
    for (const auto & iSeg : bestSegIndex)
    {
      ...
      if (seg_hasZed==0) continue; //we need Z hit
      ...
      // //if ((segTrigAMDEta < m_maxSegTrigDPhi) && (bestSegTrigAMDEta > segTrigAMDEta) && (ph2TpgThetaEmuAm_quality->at(iTrigAM) >= minQuality))
      ...
      Int_t    AMRPCflag= -1;
      Double_t bestSegTrigAMDEta = 1000;
      Double_t bestSegTrigAMDEtaNoBX = 1000;
      Double_t bestAMDEta = 0;
      Int_t    besttrigAMBX = 0;
      for (std::size_t iTrigAM = 0; iTrigAM < ph2TpgThetaEmuAm_nTrigs; ++iTrigAM)
      {
        Int_t trigAMWh  = ph2TpgThetaEmuAm_wheel->at(iTrigAM);
        Int_t trigAMSec = ph2TpgThetaEmuAm_sector->at(iTrigAM);
        Int_t trigAMSt  = ph2TpgThetaEmuAm_station->at(iTrigAM);
        Int_t trigAMBX  = ph2TpgThetaEmuAm_BX->at(iTrigAM);
        Int_t trigAMqual  = ph2TpgThetaEmuAm_quality->at(iTrigAM);
        Int_t trigAMrpc  = ph2TpgThetaEmuAm_rpcFlag->at(iTrigAM);
        
        if (segWh == trigAMWh && segSec == trigAMSec && segSt  == trigAMSt)
        {
          /*Double_t trigGlbPhi    = trigPhiInRad(ph2TpgThetaEmuAm_phi->at(iTrigAM),trigAMSec);//tocar aqui111
          Double_t finalAMDPhi   = seg_posGlb_phi->at(iSeg) - trigGlbPhi;
          Double_t segTrigAMDPhi = abs(acos(cos(finalAMDPhi)));*/
          /*Double_t segY = (-seg_posLoc_y->at(iSeg)+120)*1000;
          Double_t amY = ph2TpgThetaEmuAm_posLoc_y_raw->at(iTrigAM);
          Double_t segTrigAMDy = abs(segY-amY);*/

          Double_t segTrigAMDEta = 0;
          if(ph2TpgThetaEmuAm_z->at(iTrigAM)>0)   
                    segTrigAMDEta = abs(seg_posGlb_eta->at(iSeg) + log(tan((atan(MBRadius[trigAMSt-1]/((ph2TpgThetaEmuAm_z->at(iTrigAM))/ZRES_CONV)))/2.)));
           else segTrigAMDEta = abs(seg_posGlb_eta->at(iSeg) + log(tan((TMath::Pi()+atan(MBRadius[trigAMSt-1]/((ph2TpgThetaEmuAm_z->at(iTrigAM))/ZRES_CONV)))/2.)));

          //abs(seg_posGlb_eta->at(iSeg) - (ph2TpgThetaEmuAm_z->at(iTrigAM))*1.2/28619);
          
          // if (abs(segWh) == 2 and abs(segSt) == 1 and print) {
          // print = false;
          // cout << event_eventNumber << endl;
          // for (std::size_t iTrigAM = 0; iTrigAM < ph2TpgThetaEmuAm_nTrigs; ++iTrigAM) {
          // cout << ph2TpgThetaEmuAm_wheel->at(iTrigAM) << " ";
          // cout << ph2TpgThetaEmuAm_sector->at(iTrigAM) << " ";
          // cout << ph2TpgThetaEmuAm_station->at(iTrigAM) << " ";
          // cout << ph2TpgThetaEmuAm_quality->at(iTrigAM) << " ";
          // cout << ph2TpgThetaEmuAm_BX->at(iTrigAM) << " ";
          // cout << ph2TpgThetaEmuAm_t0->at(iTrigAM) << " ";
          // cout << ph2TpgThetaEmuAm_posLoc_x->at(iTrigAM) << " ";
          // cout << ph2TpgThetaEmuAm_dirLoc_phi->at(iTrigAM) << endl;
          // }
          // printPh2Hits();
          // }
          
          //m_plots["hPrimPsiAM"] -> Fill( ph2TpgThetaEmuAm_dirLoc_phi->at(iTrigAM) );
          ...
          m_plots2["hSegmentPsiVSDeltaT0AM"]->Fill(  atan ( (seg_dirLoc_x->at(iSeg) / seg_dirLoc_z->at(iSeg)) ) * 360 / (2*TMath::Pi()) , ph2TpgThetaEmuAm_t0->at(iTrigAM) - 20*25 - seg_phi_t0->at(iSeg) );
          if (ph2TpgThetaEmuAm_index->at(iTrigAM) > maxIndex ) continue;
          // if (ph2TpgThetaEmuAm_quality->at(iTrigAM) < minQuality ) continue;
          ...
          if (qualityMatched && ( (ph2TpgThetaEmuAm_quality->at(iTrigAM) < LOWQ && ph2TpgThetaEmuAm_rpcFlag->at(iTrigAM)==0 ) || ( ph2TpgThetaEmuAm_quality->at(iTrigAM) ==-1 ) ) ) continue;
          ...
          //if ((segTrigAMDPhi < m_maxSegTrigDPhi) && (bestSegTrigAMDEta > segTrigAMDEta) && (ph2TpgThetaEmuAm_quality->at(iTrigAM) >= minQuality))
          if ((segTrigAMDEta < m_maxSegTrigDEta) && (trigAMBX == 20) && (bestSegTrigAMDEta > segTrigAMDEta) && (ph2TpgThetaEmuAm_quality->at(iTrigAM) >= minQuality))
          {
            bestTPAM          = iTrigAM;
            besttrigAMBX      = trigAMBX;
            bestSegTrigAMDEta = segTrigAMDEta;
            bestAMDEta        = (ph2TpgThetaEmuAm_z->at(iTrigAM))*1.2/28619;
            AMRPCflag         = ph2TpgThetaEmuAm_rpcFlag->at(iTrigAM);
          }
          if (segTrigAMDEta < m_maxSegTrigDEta && (bestSegTrigAMDEtaNoBX > segTrigAMDEta) && (ph2TpgThetaEmuAm_quality->at(iTrigAM) >= minQuality))
          {
            bestTPNoBXAM          = iTrigAM;
            bestSegTrigAMDEtaNoBX = segTrigAMDEta;
          }
        }
      }
      ...
      if (bestTPAM > -1 && seg_phi_t0->at(iSeg) > -500)
      {
        ...
        //cout << "Efficient event " <<  thisEntry << " in " << whTag << " " << secTag << " " << chambTag << " Segment hits: " << seg_phi_nHits->at(iSeg) << " Segment Position: " << seg_posLoc_x->at(iSeg) << " Primitive iTrig " << bestTPAM << " out of " <<  ph2TpgThetaEmuAm_nTrigs  << " Quality: " << ph2TpgThetaEmuAm_quality->at(bestTPAM) << " t0=" << ph2TpgThetaEmuAm_t0->at(bestTPAM)  <<endl;
        //cout << "Eficiente!" << endl;
        ...
        //cout << "Inefficient event " <<  thisEntry << " in " << whTag << " " << secTag << " " << chambTag << " Segment hits: " << seg_phi_nHits->at(iSeg) << " Segment Position: " << seg_posLoc_x->at(iSeg) << " Primitive iTrig " << bestTPAM << " out of " <<  ph2TpgThetaEmuAm_nTrigs  <<endl;
      }
  ...
  //for (unsigned int iTrig = 0; iTrig < ph2TpgThetaEmuAm_nTrigs; iTrig++)  {
  //  printMPs(iTrig);
  //  cout << "-------------------------------------------------" <<endl;
//}
}
...
{
  cout << "iTrig " << iTrig
  << " Wh:" << ph2TpgThetaEmuAm_wheel->at(iTrig)
  << " Se:" << ph2TpgThetaEmuAm_sector->at(iTrig)
  << " St:" << ph2TpgThetaEmuAm_station->at(iTrig) << endl
  << "Position :" << ph2TpgThetaEmuAm_z->at(iTrig) << endl
  << "Direction :" << ph2TpgThetaEmuAm_k->at(iTrig) << endl
  << "Time :" << ph2TpgThetaEmuAm_t0->at(iTrig) << endl
  << "Quality :" << ph2TpgThetaEmuAm_quality->at(iTrig) << endl;
}
